...
specifically because `training/advanced_models.py::fit_random_forest` and
`training/baseline.py::fit_logistic_regression` both use
`class_weight="balanced"`: that reweights samples during fitting to counter
the ~0.2% positive rate, which is exactly the kind of transformation that can
shift `predict_proba`'s output away from the true empirical frequency while
leaving rank order (and therefore every metric in metrics.py) untouched.
`api/main.py`'s `/predict` and `/predict/live` hand callers a raw
`ignition_probability` float, so this was worth checking rather than
assuming, see
docs/06-modeling-and-evaluation.md#calibration-is-ignition_probability-a-real-probability
for what was found. What was found is why those endpoints now return a
`calibrated_probability` alongside it, from the pooled isotonic calibrator
`training/export_model.py::export_current_best` fits with
`fit_isotonic_calibrator` below.
"""
...
def fit_venn_abers_calibrator(y_score: np.ndarray, y_true: np.ndarray):
    ...
    random-split leakage this project's temporal-split discipline exists to prevent
    (`docs/06-modeling-and-evaluation.md#splitting-by-time-never-randomly`). The low-level class takes
...
if __name__ == "__main__":
    ...
    # in the years already known least reliable, or is the interval claiming false confidence there
    # too? Two readings are printed, because they answer it differently. The pass/fail verdict below
    # is driven by the hardcoded `sparse_years` set (2019/2020/2022, read off the isotonic/sigmoid
    # columns above), which is what docs/06's criterion was written against. corr(width,
    # 1/positives) is reported alongside it as the version that *doesn't* depend on that hardcoding,
    # so a future feature set that shifts which years are sparse still has a usable number here.
