...
Five prediction/data endpoints, plus `GET /health` (the served model's type,
feature columns and val scores, whether a calibrator is attached, and whether
the optional multi-day bundle loaded):
...
- GET /predict/explain, same live fetch as /predict/live, plus a
  ...
  see evaluation/shap_analysis.py and
...
def _reject_if_outside_fire_season(target_date: dt.date) -> None:
    """Shared fire-season guard, see docs/07-serving.md.

    Called by /risk-map directly, and by /predict/live, /predict/live/multi-day
    and /predict/explain through the shared `_resolve_live_features` below, so
    all four reject an off-season date identically rather than via four
    almost-identical hand-duplicated checks.
...
# Wide open (`*`) by default, matching the local personal-project frontend: a
# static file opened directly in a browser (no dev server) sends no `Origin`
# header CORS can match against, so an explicit allowlist would just break it.
# Set FIRESIGHT_CORS_ORIGINS to a comma-separated list of real origins (e.g.
