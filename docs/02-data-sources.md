...
- **VIIRS**, on Suomi NPP/NOAA-20, detections since ~2012, higher spatial resolution than MODIS
(~375m vs ~1km). This project uses `VIIRS_SNPP_SP` (`ingest_firms.py:DEFAULT_SOURCE`).
...
Canadian [FBP System](glossary.md#fbp-system-canadian-forest-fire-behaviour-prediction-system) is
itself built around, and a signal genuinely independent of weather: two
...
Served as a [WFS](glossary.md#wfs-web-feature-service) polygon layer, not a small pre-clipped file
(the province-wide download is a ~4GB File Geodatabase, needing `fiona`/GDAL to read, a dependency
this project deliberately doesn't have, see [Grid & labels](03-grid-and-labels.md) for the same
tradeoff already made for the fire grid itself). `features/fuel_type.py` instead queries the WFS
endpoint per grid-cell centroid with a small bounding box, since a raw point-`INTERSECTS` filter
against this layer's BC Albers geometry doesn't
...
west/south/east/north), a deliberately small, permanent region, not a starting point for a larger
build. Per the README's stated scope, this keeps the full pipeline (ingest → grid → label → join →
model → serve) fast to iterate on and cheap to run (compute, storage, download time) as a personal
project.

FIRMS and ERA5-Land were the original two, the label source and the weather feature source this
problem needs at its core. Fuel type and terrain were added later to close a
