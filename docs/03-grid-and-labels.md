...
**Mechanically, a forward rolling max, computed the same "reverse, roll, un-reverse" way `pandas` has
no native support for a forward-looking window:** sort by `(cell_id, date)` (so each cell's rows are
