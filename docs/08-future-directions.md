...
**Status: deliberately deferred, not started.** The project has stayed scoped to the Kamloops Fire
Centre since its first commit (see [Problem framing](01-problem-framing.md) and the README's
"Decided" note) -- prove the full pipeline end to end on one region before a much larger one. Three
concrete things would need to happen before this is safe to start, not just "run the same pipeline on
a bigger bbox":

1. **`features/grid.py`'s per-row reference-latitude approximation needs correcting first.** It
   converts degrees to km using a single reference latitude per row, which is a fine approximation
   across Kamloops Fire Centre's narrow latitude range but distorts meaningfully stretched across all
   of BC's roughly 44 degrees of latitude (coast to Rockies to the far north). This is a correctness
   fix, not an optimization -- left wrong, it would silently misshape the grid rather than error out.
...
rolling-origin backtest (not one aggregate number), a monthly breakdown, and its own calibration check
-- Kamloops' results are not assumed to transfer.
...
| CAPE / convective precip | **Promoted, served** | See [feature addition](06-modeling-and-evaluation.md#adding-capeconvective_precip_mm-to-feature_columns) |
